...
#define L1 8 //Left line sensor = 0 white  1 black
#define L2 9 //right line sensor = 1 white  0 black
//IR Distance sensors
#define IR A0
...
void loop() {
  ...
  if (Serial.available() > 0){  
    ...
    if (read =='t'){
      ...
      delay(2000);
      myservo.write(90); //release block
      check = 0;
    }
...
//     check = 1.5;
     ...
     if (pickup == 3){
       ...
       Serial.println("set initial path");
       Serial.print("Go To: ");
       Serial.print(tracking_x);
       Serial.print(", ");
       Serial.println(tracking_y);
     } else if (d1 < 13 + (30 * check) && (d3 > 30 || d5 > 30)){ //d1 < 19 + (25 * check) &&
       x = tracking_x; // update current location
       y = tracking_y;
       Serial.println("New Path detected");
     ...
     } else if (d1 < 13 + (24 * check) && (d3 > 30 || d5 > 30)) {
       x = tracking_x; // update current location
       y = tracking_y;
       Serial.println("New Path detected");
...
void alignment() {
  if ((d_2 < d_6 + err && d_2 > d_6 - err) || (d_3 < d_5 + err && d_3 > d_5 - err)) {
    align = 1;
    stop();
...
void localization(){
  ...
  if (count == 0) { // (6,3)
    ...
    if (b[ind3] == 3) { // 1, 3, 0, 2
      ...
      y = 3;
      theta = (3 - ind1) * 90;
    } else if ((pickup == 0) && ((ind1 - ind2 == 1) || (ind1 - ind2 == -3))){
      x = 2;
      y = 4;
      theta = (-ind1) * 90;
    } else {
...
void adjust(){
  ...
  int a = 4; //4
  int b = 2; //2
  if (((d_2 > 21.55 + err) || (d_6 < 21.55 + err)) || (d_5 < 16)){ //right wall too far
    s1 += a;
    s2 -= b;
    right_ = true;
  } else if (((d_2 < 21.55 + err/2) || (d_6 > 21.55 + err)) || (d_3 > 16)){ //right wall too near
    s1 -= b;
    s2 += a;
    left_ = true;
  } else {
...
void detectBlockPickUp(){
  ...
  if (((IR_dist + 3 < 15) && (IR_dist + 3 ) - 3 <= (d1 + 5.5) )){
    Serial.println("Found Block");
    while(IR_dist > 3){
      Serial.println("To Block");
